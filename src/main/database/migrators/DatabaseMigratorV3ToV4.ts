...
import BufferUtils from "@main/utils/BufferUtils";
...
export default class DatabaseMigratorV3ToV4 implements DatabaseMigrator {
    ...
    public async upgrade(database: Database.Database): Promise<void> {
        const readAssays = database.prepare("SELECT id FROM assays").all() as AssayRow[];
        for (const assayId of readAssays.map(o => o.id)) {
            /*
...
class PeptideDataV0 {
    ...
    public toPeptideDataResponse(): PeptideDataResponse {
        ...
        return {
            lca: this.lca,
            // @ts-ignore (this lineage can also contain null values)
