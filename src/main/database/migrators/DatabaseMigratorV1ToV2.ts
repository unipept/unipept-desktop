...
import v1_to_v2 from "@main/database/migrations/v1_to_v2.sql?raw";
...
export default class DatabaseMigratorV1ToV2 implements DatabaseMigrator {
    ...
    private async migrateBuffers(database: Database): Promise<void> {
        ...
        const rows = database.prepare("SELECT * FROM pept2data").all() as Pept2DataRow[];
