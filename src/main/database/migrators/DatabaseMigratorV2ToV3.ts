...
import v2_to_v3 from "@main/database/migrations/v2_to_v3.sql?raw";
...
export default class DatabaseMigratorV2ToV3 implements DatabaseMigrator {
    ...
    public async upgrade(database: Database.Database): Promise<void> {
        ...
        ).all() as (AssayRow & StorageMetadataRow)[];
        ...
        for (const row of assayData) {
            ...
            ).get(row.configuration_id) as SearchConfigRow;

            const peptideTrust = database.prepare(
                "SELECT * FROM peptide_trust WHERE assay_id = ?"
            ).get(row.assay_id) as PeptideTrustRow;
